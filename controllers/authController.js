...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (currentUser.changedPasswordAfter(decoded.iat)) {
    return next(new AppError('User has chnged his password. Please loggin',401))
  }
...
exports.forgotPassword =catchAsync( async (req, res, next) => {
  ...
  try {    
    ...
    return next(new AppError('There was an error sendind the email, try it later',500))
  }
})
...
exports.resetPassword = catchAsync( async (req, res, next) => {
  ...
  // 2- set new password if token has not expires
  if (!user) {
    return next(new AppError('Token is invalid or has expired', 400))
  }
  // 3- update changedPasswordAt to current user
